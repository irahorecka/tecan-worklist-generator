...
class Sequence_csv:
    def __init__(self, company, lst, filename):
...
def worklist_gen(df):
    ...
    dna_str = ""
    dil_str = ""
    seqtech_list = ['Well,Template,Primer \n']
    na_list = list()
    belowconc_list = list()
    below20_list = list()
    belowvol_list = list()
    ...
    for index, row in lunatic.iterrows():
        while position < 97 and dest_position < 97:
            if 'trough' in dil_type_source.lower():
                if position_dil > 8:
                    position_dil = 1
            #first screen to check for NA or insufficient concentration in source DNA
            if type(row['A260 Concentration\n(ng/ul)']) == float:
                na_list.append(row['Plate\nPosition'])
                break
            elif (float(row['A260 Concentration\n(ng/ul)']) / dilution) < 0.8:
                ...
                volume_dna = 1.0 * ((final_vol * dilution) / row_val)
                volume_dil = final_vol - volume_dna
                volume_dna_tot = volume_dna + sequence_vol
                #second screen to check for insufficient volume in source DNA
                if volume_dna_tot >= elution_v:
                    belowvol_list.append(row['Plate\nPosition'])
                    break
                if volume_dna > final_vol:
                    volume_dna = final_vol
                    volume_dil = 0
                
                dna_str += (f"A;{rack_source};;{rack_type_source};{str(position)};;{str('%.2f' % (volume_dna + sequence_vol))};;;\n" + 
                            f"D;{rack_seq};;{rack_type_seq};{str(dest_position)};;{str('%.2f' % (sequence_vol))};;;\n" +
                            f"D;{rack_dest};;{rack_type_dest};{str(dest_position)};;{str('%.2f' % (volume_dna))};;;\n" +
                            "W;\n")
                dil_str += (f"A;{dil_source};;{dil_type_source};{str(position_dil)};;{str('%.2f' % volume_dil)};;;\n" +  
                            f"D;{rack_dest};;{rack_type_dest};{str(dest_position)};;{str('%.2f' % volume_dil)};;;\n" +
                            "W;\n")   

                if seq_company.lower() == "sequetech":
    ...
    warning_str = "" 
    range_str = ""
    if len(belowconc_str) + len(na_str) + len(belowvol_str) != 0:
        ...
        if len(belowvol_str) != 0:
            warning_str += "ELUTIION VOL. < REQUIRED VOL.: \n" + belowvol_str + "\n\n"
    if len(caut_str) != 0:
    ...
    return(dna_str, dil_str, warning_str, range_str, seqtech_list)
...
for filename in os.listdir(os.getcwd()):
    if filename[-5:] == '.xlsx':
        try:
            ...
            csv_file = os.path.join(csv_direct, str(filename[:-5]) + '.csv')
            dna, dil ,warn, out, seqtech = worklist_gen(csv_file)
        except KeyError:
            continue
        ...
        sequetech = Sequence_csv("sequetech", seqtech, filename)
        if len(seqtech) > 1:
            os.chdir(new_direct)
            sequetech.csv_writer()
            os.chdir(xlsx_direct)
        if dna != "":
